...
$module = str_replace('_', ' ', 'product_colors');
$module_name_full = 'product_colors';
$module_name = $module;
$hide_col = ['id', 'sn'];
?>
        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-container  kt-container--fluid ">
               ...
               <div class="kt-subheader__toolbar">
                  <div class="btn-group">
                      <?php echo add_new(null, 'admin/' . $module_name_full . '/form'); ?>
                      <?php echo delete_all($module_name_full); ?>
                      <?php echo export(null, 'admin/' . $module_name_full . '/export_csv'); ?>
                      <?php echo import(null, 'admin/' . $module_name_full . '/import_csv'); ?>
                  </div>
               </div>
            </div>
        </div>
                                   ...
                                   <tr>
                                      <td>
                                       ...
                                       foreach ($row as $col => $value) {
                                           ...
                                           switch ($col) {
                                               /*case 'color_name';
                                                   echo '<td>' . detail_input('color_name',$row->id,$row->color_name,'product_colors/AJAX/color/','color') . '</td>';
                                                   break;*/
                                               case 'color_name';
                                                   echo '<td>' . $row->color_name . '</td>';
                                                   break;
                                               case 'color_code'; ?>
                                                   <td class="text-center"><div style="width:25px;height:25px;background:#<?php echo $row->color_code; ?>;margin:0 auto;border-radius:50%" data-skin="dark" data-toggle="kt-tooltip" title="#<?php echo $row->color_code; ?>"></div></td>
                                                   <?php break;
                                               case 'status';
                                                   echo '<td>' . data_status($row->status) . '</td>';
                                                   break;
                                               case 'ordering';
                                                   echo '<td>' . ordering_input($module_name_full.'/AJAX/ordering/',$row->id,$row->ordering) . '</td>';
                                                   break;
                                               default:
                                                   echo '<td>' . $value . '</td>';
                                           }
                                       ...
                                       <?php } ?>
                                      <td>
                                          <?php echo _edit('admin/' . $module_name_full . '/form/' . $row->id); ?>
                                          <?php echo _delete('admin/' . $module_name_full . '/delete/' . $row->id); ?>
                                          <?php echo _status('admin/' . $module_name_full . '/status/' . $row->id . '?status=' . $row->status, $row->status); ?>
                                      </td>
                                   </tr>
                                    <?php $s++; } ?>
                                </tbody>

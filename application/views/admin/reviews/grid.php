...
$module_name = 'reviews';
$hide_col = ['id', 'sn','email','full_name','product_image', 'product_name', 'friendly_url'];
?>
    <div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
        <!-- begin:: breadcrumb -->
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-container  kt-container--fluid ">
                <?php echo breadcrum('Product Reviews', $module_name.LIST_TEXT); ?>
                                      ...
                                      <td>
                                       ...
                                       foreach ($row as $col => $value) {
                                           ...
                                           if ($row->product_image != '') {
                                               $image = fancyImg_round('images/products/' . $row->product_image, 50, 50, '', null);
                                           } else {
                                               $image = noImg();
                                           }

                                           //$pro_link = '<a href="'.site_url('products/'.$row->friendly_url.'-'.$row->id).'" target="_blank">'.$row->product_name.'</a>';
                                           ...
                                           switch ($col) {
                                               ...
                                               case 'status';
                                                   ...
                                                   break;
                                               case 'star_rating';
                                                   echo '<td>' . $row->star_rating .' Stars'. '</td>';
                                                   break;
                                               case 'created';
                                                   echo '<td>' . date('M d, Y',strtotime($row->created)) . '</td>';
                                                   break;
                                               case 'reviews';
                                                   echo '<td class="text-left">' . $row->reviews . '</td>';
                                                   break;
                                               default:
                                                   echo '<td>' . $value . '</td>';
                                           }
                                       ...
                                       <?php } ?>
                                      <td>
                                         <a href="<?php echo base_url('admin/'.$module_name.'/view/'.$row->id); ?>" data-skin="dark" data-toggle="kt-tooltip" title="View"> <i class="la la-eye kt-font-primary"></i> </a>

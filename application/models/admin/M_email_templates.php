...
class M_email_templates extends CI_Model
{
    ...
    function insert()
    {
        ...
        $db_data = getDbArray('email_templates');
        ...
        $_id = save('email_templates', $db_data['dbdata'], ($id > 0 ? 'id=' . $id : ''));
    ...
    {
        $this->db->delete('email_templates', ['id' => $email_templates_id]);
    }
    ...
    {
        $ids = getVar('ids');
        $this->db->where_in('id', $ids)->delete('email_templates');
    }
    ...
    function status()
    {
        ...
        $SQL = "UPDATE email_templates SET `status` = '$_status' WHERE id = {$id}";
        $this->db->query($SQL);
    }

...
class M_home_page extends CI_Model
{
    ...
    function insert()
    {
        ...
        $db_options = array_column($this->db->get('one_page')->result_array(), 'option_name');

        foreach ($option as $key => $value) {
            if (!in_array($key, $db_options)) {
                $this->db->insert($this->table, ['option_name' => $key, 'option_value' => $value]);
            } else{
                $this->db->update($this->table, ['option_value' => $value], ['option_name' => $key]);
            }
        }
        ...
        foreach ($delete_img as $col => $item) {
            if (!empty($item))
                $this->db->update($this->table, ['option_value' => ''], ['option_name' => $col]);
        }
        ...
        foreach ($_FILES as $_file_column => $FILE) {
            ...
            if (!empty($_FILES[$_file_column]['name'])) {
                ...
                if (!$upload['status']) {
                    ...
                    $key  = $_file_column;
                    if (!in_array($key, $db_options)) {
                        $this->db->insert($this->table, ['option_name' => $key, 'option_value' => $upload['upload_data']['file_name']]);
                    } else{
                        $this->db->update($this->table, ['option_value' => $upload['upload_data']['file_name']], ['option_name' => $key]);
                    }
                }
            }
        }

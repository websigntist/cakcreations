...
class M_slider extends CI_Model
{
    ...
    function insert()
    {
        ...
        $_id = save('slider', $db_data['dbdata'], ($id > 0 ? 'id=' . $id : ''));
    ...
    {
        $this->db->delete('slider', ['id' => $user_id]);
    }
    ...
    {
        $ids = getVar('ids');
        $this->db->where_in('id', $ids)->delete('slider');
    }
    ...
    function status()
    {
        ...
        $SQL = "UPDATE slider SET `status` = '$_status' WHERE id = {$id}";
        $this->db->query($SQL);
    }
